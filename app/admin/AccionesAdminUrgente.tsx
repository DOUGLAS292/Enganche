...
export default function AccionesAdminUrgente({ impulsoId }: { impulsoId: string }) {
  ...
  async function accionar(accion: "confirmar" | "rechazar") {
    ...
    setCargando(true);
    try {
      const res = await fetch(`/api/admin/urgentes/${impulsoId}/${accion}`, { method: "POST" });
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        >
          {cargando ? "…" : "Confirmar pago y avisar"}
        </button>
        <button
